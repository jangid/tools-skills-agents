...
### Q-IMPL-019: overview.md keeps `status: Approved` through the Chunk 7 v4 doc update
**Tier**: 2 (spec ambiguity)
**Spec reference**: § Deferred work — "`docs/spec/overview.md` is Approved and is not edited in the specs pass, but the update is mandatory before v4 ships." The note mandates the edit but is silent on whether applying it flips `overview.md`'s `status` off `Approved`.
**Decision**: Applied the mandated §Version Marker (`4` valid value) and §ID Namespaces (`<WS>` segment) edits and bumped `last_updated` to 2026-07-23, but **retained `status: Approved`** rather than dropping to `Draft`. Chunk 7 records this as a documentation update of the already-approved shipped v4 contract (the behavior it documents was approved and implemented across Chunks 0–6), not a new design decision requiring re-approval; no `sdd-specs` re-approval gate is crossed within this implement cycle. Also added a `Q-IMPL-NNN` / `Q-IMPL-<WS>-NNN` row to the overview §ID Namespaces tables (the table previously listed only three namespaces, omitting Q-IMPL) so the doc matches `ws-ids.md`'s three ws-prefixed downstream ids.
**Rationale**: The Deferred-work note frames this strictly as bringing an Approved doc into line with an already-shipped contract, not as re-opening its design. Flipping to `Draft` would spuriously signal the shared overview is unsettled and could route a phase-detecting skill into `sdd-specs`. Keeping `Approved` + bumping `last_updated` preserves accurate staleness signalling while reflecting that the contract is settled. Default carried: retain `Approved` for in-cycle shipped-contract doc updates; a real design change would instead go through `sdd-specs` re-approval.
